...
class VendorListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        for vendor in queryset:
            vendor.on_time_delivery_rate = vendor.purchaseorder_set.filter(status='completed', delivery_date__lte=('delivery_date')).count() / vendor.purchaseorder_set.filter(status='completed').count() * 100 if vendor.purchaseorder_set.filter(status='completed').count() > 0 else 0.0
            ...
            vendor.fulfillment_rate = vendor.purchaseorder_set.filter(status='completed', issue_date__lte=('acknowledgment_date')).count() / vendor.purchaseorder_set.count() * 100 if vendor.purchaseorder_set.count() > 0 else 0.0
        return queryset
...
class PurchaseOrderListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if serializer.validated_data['status'] == 'completed':
            serializer.instance.vendo.quality_rating_avg = serializer.instance.calculate_quality_rating_avg()
